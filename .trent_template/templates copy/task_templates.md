...
> **Full reference**: See `task_template.md` for the comprehensive vNext task spec with all
> autonomy fields (claim TTL, blast radius, failure history, cost tracking, spec versioning).
> This file provides simplified copy-paste templates for common task types.
...
- `pending` - Not started (task file exists, ready to claim)
- `in-progress` - Currently working (agent has claimed)
- `awaiting-verification` - Implementation done, reviewer pending
- `completed` - Done and verified
- `failed` - Attempted but failed
- `paused` - Deprioritized mid-work
- `harvested` - Approach superseded, preserved as reference
- `resource-gated` - Waiting on external resource
