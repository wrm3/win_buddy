...
**File**: `.claude/agents/sdk/fallback.py`
...
**File**: `.claude/agents/sdk/health.py`
...
# Check readiness
python .claude/agents/sdk/health.py readiness

# Validate agent
python .claude/agents/sdk/health.py validate backend-developer
```
...
handler = FallbackHandler(
    log_dir=Path(".claude/agents/sdk/logs"),
    enable_retry=True,
    max_retries=3,
    context_dir=Path(".claude/agent_context/json")
)
...
**Location**: `.claude/agents/sdk/logs/`
...
2025-11-01 12:05:06 - FallbackHandler - INFO - Context converted to JSON: .claude/agent_context/json/123.json
...
```bash
# Run test suite
python .claude/agents/sdk/examples/fallback_scenarios.py
```
...
find .claude/agent_context/json -type f -mtime +7 -delete
```
...
```bash
# Check agent files
ls -la .claude/agents/prompt-based/
...
# Check logs
tail -f .claude/agents/sdk/logs/fallback.log
