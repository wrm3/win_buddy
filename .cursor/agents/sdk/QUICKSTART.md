...
```bash
cd /mnt/c/git/trent
pip install -r .cursor/agents/sdk/requirements.txt
```
...
**What happens**: Automatic fallback to `.cursor/agents/backend-developer.md`
...
Context file: .cursor/agent_context/json/scenario1.json
...
```
...
```bash
tail -20 .cursor/agents/sdk/logs/fallback.log
```

### View Health History

```bash
tail -5 .cursor/agents/sdk/logs/health_history.jsonl | jq .
```
...
ls .cursor/agents/*.md
ls .cursor/agents/prompt-based/*.md
...
mkdir -p .cursor/agent_context/json
chmod 755 .cursor/agent_context/json
```
...
- Agent files exist (`.cursor/agents/*.md`)
