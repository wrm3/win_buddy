...
export_context_report(
    context,
    f".cursor/agent_context/reports/{context.workflow_id}.md"
)
```
...
1. Create Context
   ↓
2. Active Workflow (in .cursor/agent_context/json/)
   ↓
3. Complete Workflow
   ↓
4. Auto-Archive (move to .cursor/agent_context/archived/)
   ↓
...
Context automatically integrates with Cursor Memory:
...
```bash
# Test schema
python .cursor/agents/sdk/context/schema.py

# Test manager
python .cursor/agents/sdk/context/manager.py

# Test adapter
python .cursor/agents/prompt-based/context_adapter.py

# Test utilities
python .cursor/agents/sdk/context/utils.py
```
