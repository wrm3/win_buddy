...
echo "[$(date)] $AGENT completed: $TASK ($STATUS)" >> .cursor/logs/completions.log
...
METRICS_FILE=".cursor/metrics/agent_metrics.jsonl"
mkdir -p .cursor/metrics
...
if [ "$EVENT" = "agent-complete" ]; then
    ...
    echo "$(date),agent_files_created,$FILES_CREATED" >> .cursor/metrics/timeseries.csv
fi
