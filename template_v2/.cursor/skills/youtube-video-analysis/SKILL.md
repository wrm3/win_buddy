...
After `video_analyze` returns, **always** write a vault summary note. The agent writes this directly — no MCP needed.

### Post-Analysis Vault Write

After receiving the analysis result, write a file to the vault:

**Path**: `{vault}/videos/summaries/{date}_{safe_title}.md`
...
## Transcript Summary
{condensed summary of transcript}

## Visual Analysis
{summary of frame analysis if available}

## Timestamps
- 0:00 - {topic}
- {more timestamps from segments}

## Relevance
{how this relates to current projects}
```

### Vault Path Resolution & Dual-Write
...
1. **Project copy**: `{vault}/projects/{project_id}_{project_name}/videos/{date}_{safe_title}.md`
2. **Global copy**: `{vault}/videos/summaries/{date}_{safe_title}.md`

The project copy ensures the project always has access to its video research. The global copy makes it discoverable by other projects.

### Safe Filename

Convert the video title to a safe filename:
- Lowercase, replace spaces with hyphens
- Remove special characters
- Truncate to 80 characters
- Prefix with date: `2026-03-20_how-to-build-mcp-servers.md`
