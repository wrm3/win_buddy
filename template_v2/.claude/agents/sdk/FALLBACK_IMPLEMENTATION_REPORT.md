...
Convert to JSON
    ↓
Write to file: .claude/agent_context/json/{workflow_id}.json
    ↓
...
handler = FallbackHandler(
    log_dir=Path(".claude/agents/sdk/logs"),      # Log directory
    enable_retry=True,                             # Enable retries
    max_retries=2,                                 # Max retry attempts
    context_dir=Path(".claude/agent_context/json") # Context storage
)
```
...
3. **Monitor logs**: Watch `.claude/agents/sdk/logs/` during development
...
# Run tests
pytest .claude/agents/sdk/tests/test_fallback.py -v
